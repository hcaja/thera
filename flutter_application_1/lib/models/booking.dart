...
class ParentDailyTimeSlot {
  DateTime date;
  List<ParentTimeData> timeslot;
  ...
  Map<DateTime, List<ParentTimeData>> toMap() {
    return {date: timeslot};
  }

  factory ParentDailyTimeSlot.fromJson(Map<String, dynamic> json) =>
      ParentDailyTimeSlot(
        date: DateFormat('yyyy-MM-ddTHH:mm:ss')
            .parse(json["DATE"], true)
            .toLocal(),
        timeslot: List<ParentTimeData>.from(
            json["TIMESLOT"].map((x) => TimeData.fromJson(x))),
      );
...
class ParentTimeData {
  ...
  factory ParentTimeData.fromJson(Map<String, dynamic> json) {
    return ParentTimeData(
      id: json['ID'],
